...
PTHd = listdir("D:/openfoamData/dambreak_cases")
...
def saveh5(lrSavepth, hrSavepth):
    lrSortList = sorted([i for i in listdir(lrSavepth)], key=lambda x: \
                                    float(x.split("-")[0].split("_")[0])*10 + \
                                    float(x.split("-")[-1].replace(".npy", "")))
    hrSortList = sorted([i for i in listdir(hrSavepth)], key=lambda x: \
                                    float(x.split("-")[0].split("_")[0])*10 + \
                                    float(x.split("-")[-1].replace(".npy", "")))
    Data = {}
    for i in tqdm(files):
...
water = h5py.File("newcodeh5data/alpha.water.h5", "r")
p = h5py.File("newcodeh5data/p.h5", "r")
p_rgh = h5py.File("newcodeh5data/p_rgh.h5", "r")

# saveh5(LRSAVEPTH, HRSAVEPTH)
#
# buildall(lowres, False, LRSAVEPTH)
# buildall(highres, True, HRSAVEPTH)
